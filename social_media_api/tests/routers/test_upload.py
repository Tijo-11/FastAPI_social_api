...
@pytest.fixture
def sample_image(fs) -> pathlib.Path:  # fs is from pytest-fakefs
    path = (pathlib.Path(__file__).parent / "assets" / "myfile.png").resolve()
    fs.create_file(path)  # create fake file
    return path


# Mock the Backblaze Upload
...
def aiofiles_mock_open(mocker, fs):
    ...
    async def async_file_open(fname: str, mode: str = "r"):
        # Use the real (already patched) built-in open()
